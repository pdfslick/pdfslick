...
const FreetextMenu = ({ store }: TFreetextMenuProps) => {
  ...
  return (
    ...
    >
      <button
        class={`enabled:hover:text-black text-slate-600 p-0.5 disabled:text-slate-300 rounded-sm transition-all group relative focus:border-blue-400 focus:ring-0 focus:shadow outline-none border border-transparent`}
        onClick={() => {
